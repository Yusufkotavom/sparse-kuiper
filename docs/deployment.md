...
- Jika log menampilkan `failed to resolve host 'dpg-...-a'`, gunakan ulang `DATABASE_URL` dari menu **Connect** database (utamakan External Database URL untuk menghindari masalah DNS internal).
- Untuk Google OAuth secret, gunakan file `.json` (bukan `.js`). Runtime backend akan membaca secret files dari `/etc/secrets` dan menyalin ke folder `config/*_secrets`.
