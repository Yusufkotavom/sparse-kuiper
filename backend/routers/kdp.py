...
async def trigger_kdp_generation_grok2api(project_name: str, req: GenerateGrok2ApiImagesRequest):
    try:
        ...
        payload_path = project_dir / f".grok2api-image-job-{uuid.uuid4().hex}.json"
        ...
        process = subprocess.Popen(
            [sys.executable, "-m", "backend.services.grok2api_image_worker", str(payload_path)],
            cwd=str(BASE_DIR),
            **kwargs,
        )
        message = f"Grok2API image job started for '{project_name}'. PID: {process.pid}. Refresh assets later to see results."
        return {
            "status": "started",
            "message": message,
            "created": [],
