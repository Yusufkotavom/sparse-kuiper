"""
TikTok Uploader service — launches tiktok_upload_worker.py as a subprocess.

This avoids Windows ProactorEventLoop issues since tiktok-uploader uses
Playwright internally and must run in its own process.

Authentication:
    Uses cookies.txt (Netscape format) from: data/sessions/<account_id>/cookies.txt
    Cookies are saved when user logs in via the Accounts page.
"""
...
WORKER_SCRIPT = Path(__file__).resolve().parent.parent / "tiktok_upload_worker.py"
...
async def upload_to_tiktok(
    account_id: str,
    videos: List[Dict[str, str]], # Expected keys: "video_path", "description", "schedule" (opt), "product_id" (opt)
    headless: bool = True,
    pw_debug: bool = False,
) -> dict:
    """
    Uploads a batch of videos to TikTok via a detached subprocess worker.

    Args:
        account_id:  The account ID (matches the sessions folder name).
        videos:      A list of dictionaries containing video configurations.
    ...
    for vid in videos:
        ...
        # Combine title, description, and tags with proper spacing/newlines
        description_parts = []
    ...
    logger.info(f"[TikTok] Creating job with headless={headless}, debug_open={not headless}")

    with open(job_path, "w", encoding="utf-8") as f:
        json.dump(job_data, f, indent=2)

    log_path = JOBS_DIR / f"job_{job_id}.log"
    logger.info(f"[TikTok] Launching batch upload worker for job {job_id}: {len(formatted_videos)} videos")
        ...
        if result_path.exists():
            try:
                with open(result_path, "r", encoding="utf-8") as f:
                    result = json.load(f)
                logger.info(f"[TikTok] Job {job_id} result: {result}")
                return result
            except Exception as e:
                logger.error(f"[TikTok] Failed to read result for job {job_id}: {e}")
