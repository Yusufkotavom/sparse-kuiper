...
import { queueBuilderApi, accountsApi, Account } from "@/lib/api";
...
import { AlertTriangle, ArrowRight, CalendarClock, Facebook, Instagram, Loader2, Package, RefreshCw, Search, Share2, User, Video as VideoIcon, Youtube } from "lucide-react";
...
function QueueBuilderContent() {
    ...
    const accountMap = useMemo(() => {
        ...
        if (platforms.tiktok && tiktokAccountId) next.tiktok = tiktokAccountId;
        if (platforms.youtube && youtubeAccountId) next.youtube = youtubeAccountId;
        if (platforms.facebook && facebookAccountId) next.facebook = facebookAccountId;
        if (platforms.instagram && instagramAccountId) next.instagram = instagramAccountId;
        return next;
    }, [platforms, tiktokAccountId, youtubeAccountId, facebookAccountId, instagramAccountId]);
    const queueSourceItems = useMemo(
        () =>
            [...queue].sort((a, b) => {
                const aLabel = a.metadata?.title?.trim() || a.filename;
                const bLabel = b.metadata?.title?.trim() || b.filename;
                return aLabel.localeCompare(bLabel);
            }),
        [queue]
    );
    const filteredQueueSourceItems = useMemo(() => {
        const needle = queueSearch.trim().toLowerCase();
        if (!needle) return queueSourceItems;
        return queueSourceItems.filter((item) =>
            [
                item.filename,
                item.metadata?.title || "",
    ...
    }, [queueSearch, queueSourceItems]);
    const queuedCount = useMemo(
        () => queue.filter((item) => (item.worker_state || item.status || "").toLowerCase() === "queued").length,
        [queue]
    );
    const scheduledCount = useMemo(
        () => queue.filter((item) => (item.worker_state || item.status || "").toLowerCase() === "scheduled").length,
        [queue]
    );
    const runningCount = useMemo(
        () => queue.filter((item) => {
            const state = (item.worker_state || item.status || "").toLowerCase();
            return state === "running" || state === "uploading";
        }).length,
        [queue]
    );
    ...
    const syncSelectionToUrl = (files: string[]) => {
    ...
    const applySelection = (files: string[]) => {
        setSelectedFiles(files);
        syncSelectionToUrl(files);
    };
    ...
    const loadQueue = async () => {
        setIsLoading(true);
        try {
            const data = await queueBuilderApi.getQueue();
            setQueue(data.queue);
        } catch (e) {
    ...
    const handlePublish = async () => {
        ...
        if (scheduleEnabled && scheduleDate) {
            ...
            if (dt > maxFuture) {
                toast.error("Scheduled time cannot be more than 10 days in the future.");
                return;
            }
        ...
        try {
            if (selectedFiles.length === 1) {
                await queueBuilderApi.updateQueueMetadata(selectedFiles[0], {
                    title,
                    description,
                    tags,
                });
                ...
                toast.success(scheduleEnabled ? "Scheduled job created for 1 video." : "Job created for 1 video.");
            } else {
                if (selectedPlatforms.length > 1) {
                    toast.error("Bulk job setup is currently only supported when selecting exactly one platform.");
                ...
                await Promise.all(
                    videos.map((video) =>
                        queueBuilderApi.updateQueueMetadata(video.filename, {
                            title: video.title,
                ...
                await queueBuilderApi.bulkUpdateQueueConfig({
                    filenames: videos.map((video) => video.filename),
                    platforms: selectedPlatforms,
                    account_map: accountMap,
                ...
                toast.success(`${selectedFiles.length} jobs created successfully.`);
            }
            await loadQueue();
            router.push(scheduleEnabled ? "/runs?intent=publisher&tab=scheduled" : "/runs?intent=publisher&tab=active");
        } catch (e) {
        ...
        <div className="mx-auto max-w-7xl space-y-5 px-4 py-4 sm:px-6 sm:py-6">
            ...
            <Card className="border-border bg-surface">
                <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
                    <div className="space-y-1">
                        <p className="text-sm font-semibold text-foreground">Worker Status</p>
                        <p className="text-xs text-muted-foreground">
                            Ringkasan cepat untuk melihat apakah antrean sehat sebelum membuat job baru.
                        </p>
                    </div>
                    <div className="flex flex-wrap gap-2 text-[11px]">
                        <span className="rounded-full border border-border bg-background px-3 py-1 text-foreground">Queued {queuedCount}</span>
                        <span className="rounded-full border border-border bg-background px-3 py-1 text-foreground">Scheduled {scheduledCount}</span>
                        <span className="rounded-full border border-border bg-background px-3 py-1 text-foreground">Running {runningCount}</span>
                        <span className={`rounded-full border px-3 py-1 ${failedCount > 0 ? "border-red-500/40 bg-red-500/10 text-red-300" : "border-border bg-background text-foreground"}`}>
                            Failed {failedCount}
                        </span>
                    </div>
                </CardContent>
            ...
            <Card className="border-border bg-surface">
                <CardHeader className="pb-3">
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <div>
                            <CardTitle className="text-base">Queue Source Picker</CardTitle>
                            <CardDescription>
                                Pilih asset yang sudah ada di queue. User bisa masuk ke halaman ini langsung tanpa harus datang dari halaman lain.
                            </CardDescription>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                            <Button
                                size="sm"
                                variant="outline"
                                onClick={() => applySelection(queueSourceItems.map((item) => item.filename))}
                                disabled={queueSourceItems.length === 0}
                            >
                                Select All
                            </Button>
                            <Button
                ...
                <CardContent>
                    {queueSourceItems.length === 0 ? (
                        <EmptyState
                            icon={Package}
                            title="Queue source masih kosong"
                            description="Tambahkan asset dari Project Manager atau Scraper dulu, lalu kembali ke Queue Builder untuk memilih item yang ingin dibuat job."
                            action={{ label: "Open Project Manager", onClick: () => router.push("/project-manager") }}
                            secondaryAction={{ label: "Open Scraper", onClick: () => router.push("/scraper/downloads") }}
                        />
                    ) : (
                        <div className="space-y-3">
                            <div className="relative">
                                ...
                                <Input
                                    ...
                                    placeholder="Search by title, filename, tag, status..."
                                    className="pl-9"
                                />
                            </div>
                            {filteredQueueSourceItems.length === 0 ? (
                                <EmptyState
                                    icon={Search}
                                    title="Tidak ada item yang cocok"
                                    description="Coba ubah kata kunci pencarian atau refresh queue source."
                                    action={{ label: "Clear Search", onClick: () => setQueueSearch("") }}
                                />
                            ) : (
                                <div className="grid gap-2 md:grid-cols-2">
                                    {filteredQueueSourceItems.map((item) => {
                                const isSelected = selectedFiles.includes(item.filename);
                                const titleText = item.metadata?.title?.trim() || item.filename;
                                const stateText = item.worker_state || item.status || "pending";
                                return (
                                    <button
                                        key={item.filename}
                                        type="button"
                                        onClick={() => toggleSelectedFile(item.filename)}
                                        className={`rounded-xl border p-3 text-left transition-all ${
                                            isSelected
                                                ? "border-primary bg-primary/10"
                                                : "border-border bg-background hover:border-border-hover hover:bg-elevated/60"
                                        }`}
                                    >
                                        <div className="flex items-start justify-between gap-3">
                                            <div className="min-w-0">
                                                <p className="truncate text-sm font-semibold text-foreground">{titleText}</p>
                                                <p className="mt-1 truncate font-mono text-[11px] text-muted-foreground">{item.filename}</p>
                                            </div>
                                            <StatusBadge status={stateText} className="shrink-0" />
                                        </div>
                                        <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
                                            <span>{isSelected ? "Selected" : "Click to select"}</span>
                                            {item.scheduled_at ? <span>Schedule: {new Date(item.scheduled_at).toLocaleString()}</span> : null}
                                            {item.last_error ? <span className="inline-flex items-center gap-1 text-red-300"><AlertTriangle className="h-3 w-3" /> Retry needed</span> : null}
                                        </div>
                                        {item.last_error ? (
                                            <div className="mt-3 rounded-lg border border-red-500/20 bg-red-500/10 px-3 py-2 text-[11px] text-red-200">
                                                {item.last_error}
                                            </div>
                                        ) : null}
                                    </button>
            ...
            {selectedFiles.length === 0 ? (
                <EmptyState
                    ...
                    description="Pilih item dari Queue Source Picker di atas, atau mulai dari Assets/Runs jika ingin menambah source baru."
