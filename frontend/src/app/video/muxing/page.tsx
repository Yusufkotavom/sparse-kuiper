...
import { ExternalLink, RefreshCw, Maximize2, Minimize2, Info, Play, Square, Loader2 } from "lucide-react";

const STREAMLIT_URL = process.env.NEXT_PUBLIC_STREAMLIT_URL || "http://localhost:8501";
const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api/v1";

export default function MuxingPage() {
    const [iframeSrc, setIframeSrc] = useState(STREAMLIT_URL);
    ...
    const checkStatus = useCallback(async () => {
        try {
            const res = await fetch(`${API_URL}/services/status/autocrop`);
    ...
    const handleStart = useCallback(async () => {
        ...
        try {
            const res = await fetch(`${API_URL}/services/start`, {
                ...
                body: JSON.stringify({ name: "autocrop" }),
            });
            if (res.ok) {
                setTimeout(() => {
                    checkStatus();
                    setKey((k) => k + 1);
                    setIframeSrc(STREAMLIT_URL);
                }, 3000);
            } else {
                setStatus("stopped");
            }
        ...
        <div className={`flex flex-col ${isFullscreen ? "fixed inset-0 z-50 bg-background" : "h-[calc(100vh-0px)]"}`}>
            ...
            <div className="flex items-center justify-between px-4 py-2.5 bg-card border-b border-border shrink-0">
                <div className="flex items-center gap-2.5">
                    ...
                    <h2 className="text-sm font-semibold text-foreground">🔁 Video Muxing Studio</h2>
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    <div className="group relative">
                        ...
                        <div className="absolute right-0 top-8 w-64 bg-popover border border-border rounded-lg p-3 text-[11px] text-muted-foreground leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 shadow-xl">
                            <p className="font-semibold text-foreground mb-1">AI Video Looper Pro</p>
                            <ul className="space-y-0.5">
                                <li>✂️ Potong awal video AI</li>
                                <li>🔁 Seamless crossfade loop</li>
                                <li>⚡ Instant multiply (FFmpeg copy)</li>
                                <li>🎵 Custom audio + brainwave gen</li>
                                <li>📦 Bulk folder processing</li>
                            </ul>
            ...
            <div className="flex-1 relative bg-background overflow-hidden">
                {status === "running" ? (
                    <iframe
                        ...
                        title="AI Video Looper Pro - Streamlit"
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-background text-muted-foreground gap-4">
                        ...
                        <div className="text-center">
                            <p className="font-semibold text-foreground text-lg">
                                {status === "starting" ? "Memulai Layanan..." : 
                                 status === "stopping" ? "Menghentikan Layanan..." :
                                 "Streamlit Tidak Aktif"}
                            </p>
                            <p className="text-sm text-muted-foreground max-w-sm mt-1">
                                {status === "starting" ? "Menyiapkan AI Video Looper Pro, mohon tunggu sebentar." :
                                 status === "stopped" ? "Klik tombol 'Start' di atas untuk menjalankan studio muxing." :
